        ...
        <section class="section">
          ...
          <div class="pic-wrapper" ref="pics">
            <ul class="pic-list" ref="ul">
              <li class="pic-item" v-for="pic in shopInfo.pics">
...
<script>
  ...
  export default {
   ...
   mounted() {
    ...
    watch: {
      shopInfo() { // info状态数据更新了, 但界面还没有真正更新
        this.$nextTick(() => {
         this._initScroll()
        })
      }
    ...
    computed: {
      /*数据的获取是异步的 刚开始shopInfo是空的。用这个空的去展示页面*/
      /*页面展示完后，拿到了数据。这个时候mounted已经执行完了*/
      ...mapState(['shopInfo']),
    },
    methods:{
      _initScroll(){
        this.ratingsSroll = new BScroll(this.$refs.info, {click: true})
        /*动态创建ul的长度 ul的长度必须大于div才行*/
        this.$refs.ul.style.width=120*this.shopInfo.pics.length+5*6+"px"
